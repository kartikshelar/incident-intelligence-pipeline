"""Incident record schema v0.1 — the extraction contract.
...
Schema-shape rules imposed by the structured-output API (see
app/extract/llm.py): every object forbids extra keys, every field is
required (nullable fields are `X | None`, never defaulted), no recursion.
Client-side-only constraints (min/max length, ranges, patterns) are kept
here for validation and stripped from the wire schema — that's the
retry-with-validation-error path doing its job, not the API's.
"""
...
def wire_schema() -> dict[str, Any]:
    """JSON schema for `ExtractionOutput` in the shape the API accepts."""
