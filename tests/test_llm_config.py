...
def test_factory_builds_the_configured_provider_model_and_thinking() -> None:
    cfg = _settings(
        llm_provider="anthropic",
        extraction_model="configured-model",
        extraction_thinking="adaptive:low",
    ...
    assert client.thinking == "adaptive:low"  # verbatim: it is part of the row's identity
...
def test_unset_provider_is_a_recorded_configuration_error() -> None:
    with pytest.raises(PermanentExtractionError, match="APP_LLM_PROVIDER"):
        build_llm_client(_settings(extraction_model="m", extraction_thinking="default"))
...
def test_unknown_provider_is_a_recorded_configuration_error() -> None:
    with pytest.raises(PermanentExtractionError, match="unknown LLM provider 'nope'"):
        build_llm_client(
            _settings(llm_provider="nope", extraction_model="m", extraction_thinking="default")
        )
...
def test_unset_model_is_a_recorded_configuration_error() -> None:
    with pytest.raises(PermanentExtractionError, match="APP_EXTRACTION_MODEL"):
        build_llm_client(_settings(llm_provider="anthropic", extraction_thinking="default"))


def test_unset_thinking_is_a_recorded_configuration_error() -> None:
    with pytest.raises(PermanentExtractionError, match="APP_EXTRACTION_THINKING"):
        build_llm_client(_settings(llm_provider="anthropic", extraction_model="m"))


def test_invalid_thinking_is_a_recorded_configuration_error() -> None:
    cfg = _settings(
        llm_provider="anthropic",
        extraction_model="m",
        extraction_thinking="budget:1024",
        anthropic_api_key=SecretStr("k"),
    )
    with pytest.raises(PermanentExtractionError, match="APP_EXTRACTION_THINKING='budget:1024'"):
...
def test_settings_read_provider_model_thinking_and_key_from_env(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("APP_LLM_PROVIDER", "anthropic")
    monkeypatch.setenv("APP_EXTRACTION_MODEL", "  model-from-env ")
    monkeypatch.setenv("APP_EXTRACTION_THINKING", " disabled ")
    ...
    assert cfg.extraction_thinking == "disabled"
...
def test_settings_read_from_dotenv_file(tmp_path: pytest.TempPathFactory) -> None:
    ...
    env_file.write_text(
        ...
        "APP_EXTRACTION_THINKING=adaptive:medium\nANTHROPIC_API_KEY= sk-x\n"
    )
    ...
    assert cfg.extraction_thinking == "adaptive:medium"
    assert cfg.anthropic_api_key is not None
